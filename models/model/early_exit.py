    ...
    def __init__(
        ...
        self.conformer = nn.ModuleList(
            [
                ...
                for _ in range(self.n_enc_exits)
            ]
        )
...
class Early_zipformer_2layer_exits(nn.Module):
...
class Splitformer(nn.Module):
    ...
    def __init__(
        ...
        self.conv_subsample = Conv1dSubampling(
            in_channels=features_length, out_channels=d_model
        )
        self.positional_encoder = PositionalEncoding(
            d_model=d_model, dropout=drop_prob, max_len=max_len
        ...
        self.downsampling = nn.ModuleList(
            [Downsampling(self.factor) for _ in range(2)]
        )  # downsampling layers for first and last exits
        self.upsampling = nn.ModuleList(
            [Upsampling(self.factor) for _ in range(2)]
        )  # upsampling layers for first and last exits
        self.conformer_parallel = nn.ModuleList(
            [
                Conformer(
                    input_dim=self.input_dim,
                    num_heads=self.num_heads,
        ...
        )  # 2 conformer layers for parallel downsampling for first and last exits
    ...
    def forward(self, src, lengths):
        ...
        for index in range(0, self.n_enc_exits):
            ...
            if (
                index == 0 or index == self.n_enc_exits - 1
            ):  # Parallel downsampling for first and last exits

                pad = enc_downsampled.size(1) % self.factor
                if pad != 0:  # Padding if necessary
                    pad = self.factor - pad
                    padding = torch.zeros(
                        enc_downsampled.size(0),
                        pad,
                        enc_downsampled.size(2),
                        device=self.device,
                    )
                    enc_downsampled = torch.cat((enc_downsampled, padding), dim=1)

                enc_downsampled = self.downsampling[index // (self.n_enc_exits - 1)](
                    enc_downsampled
                )  # Downsampling
                length = (
                    torch.clamp(
                        (lengths + pad) / self.factor, max=enc_downsampled.size(1)
                    )
                    .to(torch.int)
                    .to(self.device)
                )

                enc_downsampled, _ = self.conformer_parallel[
                    index // (self.n_enc_exits - 1)
                ](enc_downsampled, length)

                enc_downsampled = self.upsampling[index // (self.n_enc_exits - 1)](
                    enc_downsampled
                )  # Upsampling
                ...
                length = (
                    torch.clamp(base_length, max=enc_downsampled.size(1))
                    .to(torch.int)
                    .to(self.device)
                )
...
class Early_transformer(nn.Module):
    ...
    def __init__(
        ...
        self.conv2_subsample = Conv2dSubampling(
            in_channels=features_length, out_channels=d_model
        )
        self.positional_encoder_1 = PositionalEncoding(
            d_model=d_model, dropout=drop_prob, max_len=max_len
        )
        self.positional_encoder_2 = PositionalEncoding(
            d_model=d_model, dropout=drop_prob, max_len=max_len
        )
    ...
    def forward(self, src, trg):
        ...
        tgt_key_padding_mask = self.create_pad_mask(trg, self.trg_pad_idx).to(
            self.device
        )
        trg = self.emb(trg)
        ...
        enc = src
        output, enc_out = [], []
        for linear_1, linear_2, encoder, decoder in zip(
            self.linears_1, self.linears_2, self.encoders, self.decoders
        ):
            enc = encoder(enc, src_pad_mask)

            # Decoder
            out = decoder(
                trg, enc, tgt_mask=tgt_mask, tgt_key_padding_mask=tgt_key_padding_mask
            )
            out = linear_2(out)
            out = torch.nn.functional.log_softmax(out, dim=2)
...
class full_conformer(nn.Module):
    ...
    def forward(self, src, lengths, trg):
        ...
        for linear_1, linear_2, layer, decoder in zip(
            self.linears_1, self.linears_2, self.conformer, self.decoders
        ):
            enc, _ = layer(enc, length)
